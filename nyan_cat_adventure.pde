...
void initGame(){//I wrot this part 
  items = new Item[6];
  for(int i = 0; i < items.length; i++){    
    if( newX!=TREE && newY!=TREE || newX!=CAR && newY!=CAR){
      float newX = floor(random(8));
      float newY = i * 4 + floor(random(4));
      newX = (newX+6)*80+newY*(-30);
      newY= (newX+6)*20+newY*60;
      new Coin( newX, newY);
    }
/*    switch (floor(random (2))){
      case 0: items[i]= new Coin( newX, newY); break;
